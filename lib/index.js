...
function makeResetable(reducer, resetType) {
  return (state, action) => action.type === resetType ? reducer(action.state, action) : reducer(state, action);
}

export default function createGlobalStore({ reducer, prefix = 'global', room = '/', socket = io() }, cb) {
